---
title: "凸优化-次梯度算法"
description: "凸优化系列：次梯度的定义与性质、次梯度算法及其收敛性分析，含正则化逻辑回归与随机次梯度算法实例。"
...
featured: false
language: "zh"
legacyUrl: "/2015/10/02/cmu-10725-subgradidient/"
---
...
$$
f(x_{best}^{(k)}) - f^{\ast} \leq \frac{\parallel x^{(1)} - x^{\ast} \parallel_2^2 + \sum_{i=1}^k t^2 \parallel g^{(i)} \parallel_2^2}{2tk}
$$
...
此时，如果我们想要获得$\epsilon$-最优解，则令

$$
\frac{R^2}{2tk}+ \frac{G^2t}{2} = \epsilon
$$

令等式的每一部分等于$\frac{\epsilon}{2}$，则

$$
k=\frac{R^2}{t} \cdot \frac{1}{\epsilon}=\frac{R^2G^2}{\epsilon^2}
$$
