---
title: "机器学习-自步学习"
description: "自步学习（Self-paced Learning）入门：从课程学习出发，介绍 SPL 的数学形式、相关应用与参考文献。"
...
featured: false
language: "zh"
legacyUrl: "/2017/07/24/selfpaced/"
---
...
假设训练集$D=\{(x_1, y_1), \ldots, (x_n, y_n)\}$存在$n$个样本，每个样本$x_i$代表一个样本，$y_i$表示对应样本的类别标签，$w$表示对应模型的待优化参数，参数$w$的正则项记为$r(w)$，模型的损失函数记为$f(x_i, y_i, w)$。传统机器学习方法的目标函数记为：

$$
w_{t+1} = \operatorname{argmin}(r(w) + \sum_{i=1}^n f(x_i, y_i, w)).
$$
...
$$
(w_{t+1}, v_{t+1}) = \operatorname{argmin}(r(w) + \sum_{i=1}^n v_i f(x_i, y_i, w) - \frac{1}{K} \sum_{i=1}^n v_i).
$$
