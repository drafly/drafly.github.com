...
description: "机器学习系列：逻辑回归模型、极大似然估计的目标函数推导与牛顿法求解过程。"
...
featured: false
language: "zh"
legacyUrl: "/2015/08/05/mle/"
---

在[梯度下降算法](https://www.longfeihan.cn/凸优化/2015/07/29/gradient/)一文中，我们提到了线性回归和其对应的几种优化方法，本文主要介绍逻辑回归（*Logistic Regression*）和最大似然估计。线性回归和逻辑回归是机器学习算法中最常用的两个算法，分别用于预测和分类，属于有监督学习。在有监督学习中，如果因变量为有限个离散值，则预测问题就转变成分类问题，从这个角度也可以说分类问题是预测问题的一个特例。此时，通过样本集训练获得的有监督学习模型，即逻辑回归模型就称为分类模型或分类决策函数，也称为分类器（*Classifier*）。
...
$$
\begin{aligned}
log(\ell(\theta)) & = log(\prod_{i=1}^m P(y^{(i)}=1｜x^{(i)})^{y(i)} \cdot P(y^{(i)}=0｜x^{(i)})^{1-y(i)}) \\
...
$$
\begin{aligned}
\frac{\partial J}{\partial \theta} & = -\sum_{i=1}^m ( \frac{y}{g(x)} \cdot g'(x) - \frac{1-y}{1-g(x)}\cdot g'(x)) \\
