---
title: "凸优化－优化问题"
description: "凸优化课程笔记（CMU 10-725）：优化问题的标准形式、等价变换与最优解的存在性条件。"
...
featured: false
language: "zh"
legacyUrl: "/2015/09/25/cmu-10725-optimization/"
---
...
\text{subject to} \quad & \parallel \beta \parallel_1 \leq s
\end{aligned}
$$
...
### 5. 凸优化问题的一些性质和Trick
* *First-order optimality condition*：对于凸优化问题$min \, f(x), \, \text{subject to} \, x \in C$，如果函数$f$可微，那么当且仅当满足下式时，可行解（*feasible point*）$x$为最优解。
...
\begin{aligned}
\min \limits_{\beta,\beta_0,\xi}  \quad & \frac{1}{2}\parallel \beta \parallel_2^2 + C \sum_i^n \xi_i \\
\text{subject to} \quad & \xi_i \geq 0, \, y_i(x_i^T \beta + \beta_0) \geq 1-\xi_i
\end{aligned}
$$
...
$$
min\, f(x), \, \text{subject to}\, x \in C \Longleftrightarrow min \, h(f(x)), \, \text{subject to}\, x \in C
$$
...
\begin{aligned}
\min \limits_{x} \quad & c^Tx \\
\text{subject to} \quad & x_1F_1+\ldots +x_nF_n \succeq F_0 \\
&Ax=b
\end{aligned}
$$
