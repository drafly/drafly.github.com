...
$$\theta_{n+1} := \theta_n - \alpha /(2m) * \frac{2 f'(\theta)}{f''(\theta)}=\theta_n - \alpha*\frac{\sum^m_{i=1}\,(\theta^T \cdot x^{(i)} - y^{(i)})}{\sum^m_{i=1}\, x^{(i)}}$$

上面求得的二阶导数$$\sum^m_{i=1}\, x^{(i)}$$当且仅当参数为一维时才成立，当参数为多维时，牛顿算法的迭代公式可以写成：
