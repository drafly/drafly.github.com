...
\begin{split}
 \, & \quad -u^TAx = -b^Tu \\\
+ & \quad -v^TGx \geq -h^Tv \\\
= & \quad (-A^Tu - G^Tv)^Tx \geq -b^Tu - h^Tv
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\min \limits_{\beta,\beta_0,\xi} & \quad \frac{1}{2}\parallel \beta \parallel_2^2 + C \sum \xi_i \\\
...
又因为$$v \geq 0$$，因此我们可以消除松弛因子$$v$$(*slack variable*)，SVM对偶优化问题就变为：
