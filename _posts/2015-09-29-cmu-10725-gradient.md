...
故，证明出$$f(x^{k}) - f(x^{\ast}) \leq \frac{1}{2tk} \parallel x^0-x^{\ast}\parallel_2^2 = C/k$$，如果通过梯度下降算法想要获得*$$\epsilon$$-optimal*的解，即$$f(x^{k}) - f(x^{\ast}) \leq \epsilon$$，我们至少需要$$O(1/ \epsilon)$$次迭代，因此，梯度下降算法的收敛速度为$$O(1/k)$$。
